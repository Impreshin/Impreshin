...
CKEDITOR.plugins.add('jqueryspellchecker', {
	...
	config_defaults: {
		...
		parser    : 'html',
		webservice: {
			path  : '/system/spellcheck?custom=1/my.txt',
			driver: 'Enchant'
		},
		suggestBox: {
			position: 'below',
	...
	init: function (editor) {
		...
		this.config = $.extend( this.config_defaults, editor.config.spell_checker);
	...
	create: function () {
		...
		$(this.editorWindow).on('scroll.spellchecker', $.proxy(function scroll() {
	...
	createSpellchecker: function () {
		...
		var c = this.config;
		if (typeof(	this.language) == "function") {
			c.lang = this.language;
			c.lang = c.lang();
		}

		

		t.spellchecker = new $.SpellChecker(t.editor.document.$.body, c);
