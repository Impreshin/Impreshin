...
$(document).ready(function () {
	...
	$(document).on("click","#save-btn",function(){
		...
		$.post("/ab/save/admin_accounts_import/_save/", data, function (r) {
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		$.post("/ab/data/admin_accounts_import/_details/", data, function (r) {
			if (r['data']['error'].length) {
				var str = "";
				for (var i in r['data']['error']) {
					str += '<div class="alert alert-error">' + r['data']['error'][i] + '</div>'
...
function getData() {
	...
	detailsRequest.push($.getJSON("/ab/data/admin_accounts_import/_details", "", function (data) {
		getDetails(data)
	}));
}
