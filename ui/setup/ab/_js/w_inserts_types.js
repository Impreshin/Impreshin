...
$(document).ready(function () {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.post("/setup/save/ab/inserts_types/_delete/?ID=" + ID + "&pID=" + pID + "&cID=" + cID + "&app=ab", function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/setup/save/ab/inserts_types/_save/?ID=" + ID + "&pID=" + pID + "&cID=" + cID + "&app=ab", data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
...
function getList() {
	...
	listRequest.push($.getJSON("/setup/data/ab/inserts_types/_list", {"pID": pID, "cID": cID, "app": "ab","order":order}, function (data) {
		data = data['data'];
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/setup/save/ab/inserts_types/_sort/", {"pID": pID, "cID": cID, "app": "ab","order":rec}, function (t) {
...
function getDetails() {
	...
	detailsRequest.push($.getJSON("/setup/data/ab/inserts_types/_details", {"ID":ID, "pID": pID, "cID": cID, "app": "ab"}, function (data) {
		data = data['data'];
