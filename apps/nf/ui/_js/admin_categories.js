...
$(document).ready(function () {
	...
	$(document).on("click", "#record-list .record", function (e) {
		...
		$.bbq.pushState({"ID":ID});
		if ($cur_pub.length) {
			$.post("/app/nf/save/admin/categories/_pub/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		} else {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.post("/app/nf/save/admin/categories/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/nf/save/admin/categories/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
...
function getList() {
	...
	$.getData("/app/nf/data/admin/categories/_list", {"order":order}, function (data) {
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/app/nf/save/admin/categories/_sort/", {"order":rec}, function (t) {
...
function getDetails() {
	...
	$.getData("/app/nf/data/admin/categories/_details", {"ID":ID}, function (data) {
