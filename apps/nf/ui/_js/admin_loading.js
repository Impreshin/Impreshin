...
$(document).ready(function () {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this loading?")) {
			...
			$.post("/app/nf/save/admin/loading/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/nf/save/admin/loading/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#copyfrom-btn", function () {
		...
		$.post("/app/nf/save/admin/loading/_copyfrom/?pID=" + copyfromID, "", function (r) {
...
function getList() {
	...
	$("#right-area .loadingmask").show();
	$.getData("/app/nf/data/admin/loading/_list", {"order":order}, function (data) {
...
function getDetails() {
	...
	$.getData("/app/nf/data/admin/loading/_details", {"ID":ID}, function (data) {
