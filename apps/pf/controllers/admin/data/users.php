 ...
 */
namespace apps\pf\controllers\admin\data;
...
class users extends \apps\pf\controllers\data\data {
...
//test_array($records); 
	...
	function _details(){
		...
		if (!$details['ID']){
			...
			$userPublications = \models\publications::getAll_user("pf_users_pub.uID='" . $details['ID'] . "'", "publication ASC");
		}
		...
		$permissions = \apps\pf\permissions::defaults($user['company']['ID']);
		...
		if (count($extra)) {
			$extra = $extra[0];
			if ($extra['pf_permissions']){
				$user_permissions = @unserialize($extra['pf_permissions']);
