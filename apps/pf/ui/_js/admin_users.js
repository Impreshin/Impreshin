...
$(document).ready(function () {
	...
	$(document).on("click", "#record-list .record", function (e) {
		...
		$.bbq.pushState({"ID":ID});
		if ($cur_pub.length) {
			$.post("/app/pf/admin/save/users/_pub/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		} else {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (copfirm("Are you sure you want to remove this user from the company?")) {
			$("#left-area .loadingmask").show();
			$.post("/app/pf/admin/save/users/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("click", "#btn-add-app", function () {
		...
		if (confirm("Are you sure you want to allow access to this Application?")) {
			$("#left-area .loadingmask").show();
			$.post("/app/pf/admin/save/users/add_app/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		}
	...
	$(document).on("click", "#btn-remove-app", function () {
		...
		if (confirm("Are you sure you want to remove access to this Application?")) {
			$("#left-area .loadingmask").show();
			$.post("/app/pf/admin/save/users/remove_app/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		}
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		$("#left-area .loadingmask").show();
		$.post("/app/pf/admin/save/users/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['exists']) {
				if (confirm("This user already exists. do you want to add them to the application / company?")) {
					$.post("/app/pf/admin/save/users/add_company/?ID=" + r['exists'], function () {
...
function getList() {
	...
	$.getData("/app/pf/admin/data/users/_list", {"order":order}, function (data) {
...
function getDetails() {
	...
	$.getData("/app/pf/admin/data/users/_details", {"ID":ID}, function (data) {
