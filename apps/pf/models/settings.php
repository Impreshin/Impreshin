...
class settings extends _ {
	...
	public static function _read($section) {
		...
		$settings = \apps\pf\settings::_available($user['permissions']);
		$defaults = \apps\pf\settings::defaults();
...
//test_array($settings['groupby'][$section]);
	...
	public static function save($values = array(), $uID = "") {
		...
		$t = New \DB\SQL\Mapper($f3->get("DB"),"pf_users_settings");
		$t->load("uID='$uID'");
	...
	private static function settings_dbStructure() {
		$f3 = \Base::instance();
		$table = $f3->get("DB")->exec("EXPLAIN pf_users_settings;");
		$result = array();
