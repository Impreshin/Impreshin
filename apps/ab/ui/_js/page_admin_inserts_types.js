...
$(document).ready(function () {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.post("/app/ab/save/admin_inserts_types/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/ab/save/admin_inserts_types/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#btn-log", function (e) {
		...
		logsRequest.push($.getJSON("/app/ab/logs/inserts_types", {}, function (data) {
			data = data['data'];
	...
	$(document).on("click", "#copyfrom-btn", function () {
		...
		$.post("/app/ab/save/admin_inserts_types/_copyfrom/?pID=" + copyfromID, "", function (r) {
...
function getList() {
	...
	listRequest.push($.getJSON("/app/ab/data/admin_inserts_types/_list", {"order":order}, function (data) {
		data = data['data'];
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/app/ab/save/admin_inserts_types/_sort/", {"order":rec}, function (t) {
...
function getDetails() {
	...
	detailsRequest.push($.getJSON("/app/ab/data/admin_inserts_types/_details", {"ID":ID}, function (data) {
		data = data['data'];
