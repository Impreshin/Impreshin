...
$(document).ready(function () {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.post("/app/ab/save/admin_classified_types/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/ab/save/admin_classified_types/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#btn-log", function (e) {
		...
		$.getData("/app/ab/logs/classified_types", {}, function (data) {
	...
	$(document).on("click", "#copyfrom-btn", function () {
		...
		$.post("/app/ab/save/admin_classified_types/_copyfrom/?pID=" + copyfromID, "", function (r) {
...
function getList() {
	...
	$("#right-area .loadingmask").show();
	$.getData("/app/ab/data/admin_classified_types/_list", {"order":order}, function (data) {
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/app/ab/save/admin_classified_types/_sort/", {"order":rec}, function (t) {
...
function getDetails() {
	...
	$.getData("/app/ab/data/admin_classified_types/_details", {"ID":ID}, function (data) {
