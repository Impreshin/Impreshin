...
$(document).ready(function () {
	...
	$(document).on("click", "#record-list .record", function (e) {
		...
		$.bbq.pushState({"ID":ID});
		if ($cur_pub.length) {
			$.post("/app/ab/save/admin_placing_colours/_pub/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		} else {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this Colour?")) {
			...
			$.post("/app/ab/save/admin_placing_colours/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/ab/save/admin_placing_colours/_save/?ID=" + ID + "&placingID=" + placingID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#btn-log", function (e) {
		...
		logsRequest.push($.getJSON("/app/ab/logs/placing_colours", {}, function (data) {
			data = data['data'];
...
function getList() {
	...
	listRequest.push($.getJSON("/app/ab/data/admin_placing_colours/_list", {"order":order, "placingID":placingID}, function (data) {
		data = data['data'];
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/app/ab/save/admin_placing_colours/_sort/?placingID=" + placingID, {"order":rec}, function (t) {
...
function getDetails() {
	...
	detailsRequest.push($.getJSON("/app/ab/data/admin_placing_colours/_details", {"ID":ID}, function (data) {
		data = data['data'];
