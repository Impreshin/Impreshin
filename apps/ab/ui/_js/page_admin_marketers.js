...
$(document).ready(function () {
	...
	$(document).on("click", "#record-list .record", function (e) {
		...
		$.bbq.pushState({"ID":ID});
		if ($cur_pub.length) {
			$.post("/app/ab/save/admin_marketers/_pub/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		} else {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this marketer?")) {
			...
			$.post("/app/ab/save/admin_marketers/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/ab/save/admin_marketers/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#btn-log", function (e) {
		...
		logsRequest.push($.getJSON("/app/ab/logs/marketers", {}, function (data) {
			data = data['data'];
...
function getList() {
	...
	listRequest.push($.getJSON("/app/ab/data/admin_marketers/_list", {"order":order}, function (data) {
		data = data['data'];
...
function getDetails() {
	...
	detailsRequest.push($.getJSON("/app/ab/data/admin_marketers/_details", {"ID":ID}, function (data) {
		data = data['data'];
