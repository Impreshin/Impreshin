...
$(document).ready(function () {
	...
	$(document).on("click", "#record-list .record", function (e) {
		...
		$.bbq.pushState({"ID":ID});
		if ($cur_pub.length) {
			$.post("/app/cm/admin/save/details_types/_pub/?ID=" + ID, function (r) {
				getList();
				getDetails();
			});
		} else {
	...
	$(document).on("click", "#btn-delete", function () {
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.post("/app/cm/admin/save/details_types/_delete/?ID=" + ID, function (r) {
				$.bbq.removeState("ID");
				getList();
				getDetails();
			});
	...
	$(document).on("submit", "#capture-form", function (e) {
		...
		var ID = $.bbq.getState("ID");
		$("#left-area .loadingmask").show();
		$.post("/app/cm/admin/save/details_types/_save/?ID=" + ID, data, function (r) {
			r = r['data'];
			if (r['error'].length) {
				var str = "";
	...
	$(document).on("click", "#btn-log", function (e) {
		...
		$.getData("/app/cm/logs/details_types", {}, function (data) {
...
function getList() {
	...
	$.getData("/app/cm/admin/data/details_types/_list", {"order":order}, function (data) {
		...
		$recordsList.find("tbody").sortable({
			...
			update       :function (event, ui) {
				...
				$.post("/app/cm/admin/save/details_types/_sort/", {"order":rec}, function (t) {
...
function getDetails() {
	...
	$.getData("/app/cm/admin/data/details_types/_details", {"ID":ID}, function (data) {
