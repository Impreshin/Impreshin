...
//$("#whole-area .loadingmask").show();
	...
	$(document).on("reset", "#settings-modal form", function (e) {
		...
		if (confirm("Are you sure you want to reset all these settings?")) {
			...
			$.post("/app/cm/save/list_settings/?section=watchlist&reset=columns,group,order", function () {
	...
	$(document).on("submit", "#settings-modal form", function (e) {
		...
		$("#settings-modal").addClass("loading");
		$.post("/app/cm/save/list_settings/?section=watchlist", {"columns":columns, "group":group, "groupOrder":order}, function () {
...
function getList(settings) {
	...
	$.getData("/app/cm/data/watchlist/_list", {"group": group, "groupOrder": groupOrder, "highlight": highlight, "filter": filter, "order": order, "search": search, "watched": watched, "range": range}, function (data) {
